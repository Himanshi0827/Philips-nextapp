...
function EditDiscountPricingStrategyForm({ data, onChange, onComplete }) {
  ...
  const [tierDiscounts, setTierDiscounts] = useState<any[]>([]);
  const [netDiscounts, setNetDiscounts] = useState<any[]>([]);
 
  const [notDiscountable,setNonDiscountableDiscountable]=useState(false);
  const [scaledDiscounts, setScaledDiscounts] = useState<any[]>([]);
  const [scaledVolume, setScaledVolume] = useState<any[]>([]);
  const [scaledAmount, setScaledAmount] = useState<any[]>([]);
    ...
    {/* Discount Type */}
      ...
      <div className="toggle-row" style={{paddingTop:"15px"}}>
                  ...
                  <label className="switch">
                    <input type="checkbox" checked={notDiscountable} 
                      onChange={(e: any) =>handleDiscountChange(e)}
      ...
      <div style={{ display:'flex',flexDirection:'column',gap:'10px'}}>
        {tierDiscounts.map((value, index) => index<3 && (
          ...
          onChange={(e: any) => handleTierChange(index, e.target.value)}
          onBlur={(e: any) =>handleBlur(index,e.target.value,"Tier Discount")}
    ...
    {discountType === "Net Price Override" && (
      ...
      <div className="field" key={index}>
        <label>Net Price Override Tier {index + 1}</label>
          ...
          style={{ backgroundColor:notDiscountable ? "#e0e0e0" : "white",
            cursor: notDiscountable ? "not-allowed" : "text"}}
            onChange={(e: any) => handleNetChange(index, e.target.value) }
            onBlur={(e: any) =>handleBlur(index,e.target.value,"Net Price")}
    ...
    {/* Scaled Discount % */}
      ...
      <label>Scaled Discount %, Tier {index + 1}</label>
        <input type="number" step="0.01" value={value || ""} readOnly={notDiscountable}
          style={{ backgroundColor:notDiscountable ? "#e0e0e0" : "white",
            cursor: notDiscountable ? "not-allowed" : "text"}}
          onChange={(e: any) => handleScaledDiscountChange(index, e.target.value)} 
          onBlur={(e: any) =>handleBlur(index,e.target.value,"Scaled Discount")}
      ...
      <label>Volume Threshold {index + 1}</label>
        <input type="number" step="0.01" value={value || ""} readOnly={notDiscountable}
          style={{ backgroundColor:notDiscountable ? "#e0e0e0" : "white",
            cursor: notDiscountable ? "not-allowed" : "text"}}
          onChange={(e: any) => handleScaledVolumeChange(index, e.target.value) }
          onBlur={(e: any) =>handleBlur(index,e.target.value,"Volume Threshold")}
    ...
    {/* Scaled Discount Amount */}
      ...
      <div className="field" key={index}>
        ...
        <input type="number" step="0.01" value={value || ""} readOnly={notDiscountable}
          style={{ backgroundColor:notDiscountable ? "#e0e0e0" : "white",
            cursor: notDiscountable ? "not-allowed" : "text"}}
            onChange={(e: any) => handleScaledAmountChange(index, e.target.value) }
            onBlur={(e: any) =>handleBlur(index,e.target.value,"Scaled Amount")}
