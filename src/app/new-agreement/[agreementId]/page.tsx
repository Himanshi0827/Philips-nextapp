...
import { createAgreementLineItem } from "@/lib/api/services/agreement-line-item.service";
import { queryAgreementLineItemsByAgreement } from "@/lib/api/services/query-ali.service";
import "@/styles/FormLayout.css";
import { toast } from "react-toastify";

import AgreementGroupForm from "@/components/AgreementGroupForm";
import ProductSelectionForm from "@/components/ProductSelectionForm";
import DiscountPricingStrategyForm from "@/components/DiscountPricingStrategyForm";
import AgreementHeaderInformationForm from "@/components/AgreementHeaderInformationForm";
...
const agreementId = params?.agreementId as string;
  ...
  const [agreementName] = useState<string>(() =>
    typeof window !== "undefined" ? sessionStorage.getItem("agreementName") ?? "" : ""
  );
  const [agreementHeader, setAgreementHeader] = useState<any[]>([]);
  useEffect(() => {
    const fetchAgreement = async () => {
      try {
        const data = await getAgreementById(id as string);
        console.log("Agreement Header:", data);
        setAgreementHeader((data?.[0] || null) as any);
  ...
  const updatePayload = (section: any, data: any) => {
    setPayload((prev) => ({
      ...prev,
      [section]: {
        ...prev[section],
  ...
  const getHeaderValue = (formValue: any, agreementValue: any) => {
  ...
  const addIfValid = (obj: any, fieldName: any, value: any) => {
    if (value !== "" && value !== null && value !== undefined) {
      obj[fieldName] = value;
    }
  };

  const addIfNumber = (obj: any, fieldName: any, value: any) => {
  ...
  const handleFinalSubmit = async () => {
    ...
    if (MatchProductsBy === "Product") {
      ...
      const duplicatevalues = existingALI.map((ali: any) =>
        selectedProducts.filter((sp) => sp.Id === ali.Product?.Id),
      );

      if (duplicate) {
        duplicatevalues.forEach((item: any) => {
          if (item.length >= 1) {
            toast.error(
      ...
      requests = selectedProducts.map(async (childProduct) => {
        ...
        return createAgreementLineItem(lineItemPayload as any);
      });
    }
  ...
  const mapDiscountsToPayload = (payload: any, type: any, discounts: any) => {
