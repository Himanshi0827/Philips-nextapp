...
const memberId = params?.memberId as string;
  ...
  const [member, setMember] = useState<any>(() => {
    ...
    try { return JSON.parse(sessionStorage.getItem("memberDetails") ?? "null")?.member ?? null; }
    catch { return null; }
  });
  const [agreementDetails, setAgreementDetails] = useState<any>(() => {
    if (typeof window === "undefined") return null;
    try { return JSON.parse(sessionStorage.getItem("memberDetails") ?? "null")?.agreementDetails ?? null; }
    catch { return null; }
  });
  const [agmList, setAgmList] = useState<any[]>([]);
  const [oitList, setOitList] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
const [showModal, setShowModal] = useState(false);
const [selectedAgm, setSelectedAgm] = useState<any>(null);
  useEffect(() => {
    loadDetails();
  }, [memberId]);
