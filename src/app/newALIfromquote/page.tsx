...
// import { queryGetProposal, queryGetQuoteItem} from "@/lib/api/services/query-ali.service";
// import { getLookup } from "@/lib/api/services/lookup.service";
// import { getAgreementById } from "@/lib/api/services/agreement.service";
// import { createAgreementLineItem, getAgreementLineItems, updateAgreementLineItem } from "@/lib/api/services/agreement-line-item.service";
// import { toast } from "react-toastify";

// export default function NewALIfromQuotes() {
// //   const acc = useLocation();
//   const acc=JSON.parse(sessionStorage.getItem("AccountDetails"));
...
//   const [agreement_plist, setAgreement_PList] = useState<any[]>([]);
//   const [selected_agreement_g, setSelectedAG] = useState<Record<string, any>>({});
...
//   ...(qli.APTS_BillingPlan_c ? { APTS_BillingPlan_c: qli.APTS_BillingPlan_c } : {}),
...
//       const targetAgreementId = acc.Id; // Passed from navigation state
...
//                                     (i) => i.Id === item.Id
//                                   )}
//                                   onChange={(e: any) => toggleSelect(item, e)}
//                                 />{" "}
//                               </td>
...
//                         onChange={(e: any) => setApplyDeviated(e.target.checked)}
//                       />
...
export default function NewALIfromQuotes() {
//   const acc = useLocation();
  const [acc] = useState<any>(() => {
    if (typeof window === "undefined") return null;
    try { return JSON.parse(sessionStorage.getItem("AccountDetails") ?? "null"); } catch { return null; }
  });
  console.log("Account details",acc);
  const [displayQuoteItems, setDisplayQuoteItems] = useState<any[]>([]); //new
  const [discountSelections, setDiscountSelections] = useState<Record<string, any>>({});//new
  const accountname = acc?.AccountName;
  const [open, setOpen] = useState({ quotes: true, items: true });
  const [fromdate, setFromDate] = useState(() => {
    const d = new Date();
 ...
 const pageSize = 5;
  ...
  const selectedSomeDisplayItems =
    displayQuoteItems.some((item: any) =>
      SelectedItems.some((selected) => selected.Id === item.Id)
...
if (adjType === "% Discount") {
  ...
  payload.APTS_NPO_Tier_1_c = Number(qli.APTS_Offered_Price_c_c?.Value || 0);
     ...
     navigate.push(`/${acc.Id}`);
      ...
      console.error(err);
      toast.error(`Error confirming ALIs: ${err?.message || err}`);
    }
  };
...
const validateDeviatedSelection = (items) => {
  ...
  const selectedIds = new Set(items.map((item: any) => item.Id));
  const optionRows = items.filter((item: any) => item.AdjustmentType === "Option");
  const missingParents = optionRows.filter((item: any) => {
...
console.log("Selected IDs:", selectedIds);
 ...
 const toggleSelectAllVisible = (checked) => {
    if (!checked) {
      setSelectedItems((prev) =>
        prev.filter((item: any) => !displayQuoteItems.some((visible) => visible.Id === item.Id))
      );
      return;
    }
    const next = Array.from(
      new Map([...SelectedItems, ...displayQuoteItems].map((item: any) => [item.Id, item])).values()
    );
    if (getDuplicateSummary(next).length > 0) {
      toast.error(
        applyDeviated
...
//   const toggleSelect = async (item: any, e: any) => {
//   try {
//     if (e.target.checked) {
//       setSelectedProposals((prev) => [...prev, item]);
//       const allItems = await queryGetQuoteItem(item.Id);
//       const filteredItems = allItems
//         .filter((li: any) => {
//           const notBundleOptional =
//             li.APTS_IsBundleOptional_c === false;
 
//           const notOptional =
//             li.IsOptional === false;

//           const notMenuOtt =
//             li.Product?.Solution_Classification__c !==
//             "Menu/OTT";
...
//         .map((li: any) => ({
//           ...li,
//           ProposalNumber: item.Proposal_c,
//         }));
...
//         prev.filter(
//           (quote: any) =>
//             quote.Proposal_c !== item.Id
//         )
//       );
//     }
//   } catch (err) {
//     console.error(err);
//     toast.error("Error loading quote items");
//   }
// };
                              ...
                              <tr>
                                <th className="checkbox-col">
                                  <input
                                    ...
                                    onChange={(e: any) => toggleSelectAllVisible(e.target.checked)}
                                  />
                                </th>
                                <th>Proposal Id</th>
                                <th>Line Item Id</th>
